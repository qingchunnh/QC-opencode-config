...
# Code Verification
- 完成代码编写后，默认通过手动审查方式检查代码
- 手动检查完成后，询问用户是否需要运行验证，仅在用户明确同意后，才可运行验证命令

# File Operations
- 禁止以任何直接或间接方式（包括命令、脚本，或编写并运行程序）删除、移动、重命名任何文件
- 如果确实需要执行删除、移动、重命名文件操作，必须告知用户并提供对应命令，由用户手动执行
- 用户要求开发的功能本身涉及删除、移动、重命名文件操作时，必须先询问用户并获得许可